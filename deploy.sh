...
# It git-pulls the tree it lives in.
# bash reads a script incrementally, by byte offset -- it does not slurp the file. This
# script rewrites itself mid-run, so without this wrapper execution would continue at the
# old offset in the NEW file and run a splice of two versions. Calling main() on the last
# line forces bash to parse the whole file before executing any of it.
# See race-control docs/operations/troubleshooting.md.
main() {
  git pull --ff-only
